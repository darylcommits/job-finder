...
export default function AdminUsers() {
  ...
  const filteredUsers = useMemo(() => {
    ...
    if (selectedFilter !== 'all') {
      switch (selectedFilter) {
        ...
        case 'inactive':
          filtered = filtered.filter(user => !user.is_active)
          break
        case 'unverified_employers':
          filtered = filtered.filter(user => user.role === USER_ROLES.EMPLOYER && !user.is_verified)
          break
      }
    }
    ...
    if (searchTerm.trim()) {
      ...
      filtered = filtered.filter(user =>
        ...
        user.employer_profiles?.[0]?.company_name?.toLowerCase().includes(term) ||
        user.institution_partners?.[0]?.institution_name?.toLowerCase().includes(term)
      )
    }
  ...
  const handleUserAction = async (userId, action, reason = null) => {
    try {
      ...
      toast.success(
        action === 'verify' ? 'User verified successfully!' :
        action === 'unverify' ? 'User unverified' :
        action === 'activate' ? 'User activated' :
        action === 'deactivate' ? 'User deactivated' :
        action === 'promote_admin' ? 'User promoted to admin' :
        action === 'demote_admin' ? 'Admin privileges removed' :
        'User updated'
      )
...
function UserRow({ 
  ...
  const getDisplayName = () => {
    if (user.role === USER_ROLES.EMPLOYER && user.employer_profiles?.[0]?.company_name) {
      return user.employer_profiles[0].company_name
    }
    if (user.role === USER_ROLES.INSTITUTION_PARTNER && user.institution_partners?.[0]?.institution_name) {
      return user.institution_partners[0].institution_name
    }
    return user.full_name || 'Unknown User'
  }

  const getSubtitle = () => {
    if (user.role === USER_ROLES.EMPLOYER && user.employer_profiles?.[0]) {
      return `${user.full_name} • ${user.employer_profiles[0].industry || 'Unknown Industry'}`
    }
    if (user.role === USER_ROLES.INSTITUTION_PARTNER && user.institution_partners?.[0]) {
      return `${user.full_name} • ${user.institution_partners[0].institution_type || 'Unknown Type'}`
    }
    return user.email
  }
        ...
        <div className="ml-4 flex-1">
          <div className="flex items-start justify-between">
            ...
            <div className="flex items-center space-x-2">
              ...
              {user.role === USER_ROLES.ADMIN && user.id !== user.id && (
                <button
          ...
          {isExpanded && (
            <div className="mt-4 p-4 bg-gray-50 rounded-lg">
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                ...
                {/* Role-specific Info */}
                {user.role === USER_ROLES.EMPLOYER && user.employer_profiles?.[0] && (
                  <div>
                    <h4 className="font-medium text-gray-900 mb-2">Company Information</h4>
                    <div className="space-y-2 text-sm">
                      <p><strong>Company:</strong> {user.employer_profiles[0].company_name}</p>
                      <p><strong>Industry:</strong> {user.employer_profiles[0].industry || 'Not specified'}</p>
                      <p><strong>Size:</strong> {user.employer_profiles[0].company_size || 'Not specified'}</p>
                      {user.employer_profiles[0].website_url && (
                        <p><strong>Website:</strong> 
                          <a 
                            href={user.employer_profiles[0].website_url} 
                            target="_blank" 
                ...
                {user.role === USER_ROLES.JOB_SEEKER && user.job_seeker_profiles?.[0] && (
                  <div>
                    <h4 className="font-medium text-gray-900 mb-2">Job Seeker Information</h4>
                    <div className="space-y-2 text-sm">
                      <p><strong>Experience:</strong> {user.job_seeker_profiles[0].experience_years || 0} years</p>
                      <p><strong>Expected Salary:</strong> {
                        user.job_seeker_profiles[0].expected_salary_min && user.job_seeker_profiles[0].expected_salary_max
                          ? `$${user.job_seeker_profiles[0].expected_salary_min} - $${user.job_seeker_profiles[0].expected_salary_max}`
                          : 'Not specified'
                      }</p>
                      {user.job_seeker_profiles[0].skills?.length > 0 && (
                        <div>
                          <strong>Skills:</strong>
                          <div className="flex flex-wrap gap-1 mt-1">
                            {user.job_seeker_profiles[0].skills.slice(0, 5).map((skill, index) => (
                              <span key={index} className="px-2 py-1 bg-blue-100 text-blue-800 text-xs rounded">
                                {skill}
                              </span>
                            ))}
                            {user.job_seeker_profiles[0].skills.length > 5 && (
                              <span className="px-2 py-1 bg-gray-100 text-gray-600 text-xs rounded">
                                +{user.job_seeker_profiles[0].skills.length - 5} more
                              </span>
                            )}
                          </div>
                        </div>
                ...
                {user.role === USER_ROLES.INSTITUTION_PARTNER && user.institution_partners?.[0] && (
                  <div>
                    <h4 className="font-medium text-gray-900 mb-2">Institution Information</h4>
                    <div className="space-y-2 text-sm">
                      <p><strong>Institution:</strong> {user.institution_partners[0].institution_name}</p>
                      <p><strong>Type:</strong> {user.institution_partners[0].institution_type || 'Not specified'}</p>
                      <p><strong>Contact Person:</strong> {user.institution_partners[0].contact_person || 'Not specified'}</p>
                      {user.institution_partners[0].website_url && (
                        <p><strong>Website:</strong> 
                          <a 
                            href={user.institution_partners[0].website_url} 
                            target="_blank" 
                            rel="noopener noreferrer"
                            className="text-blue-600 hover:text-blue-800 ml-1"
                          >
